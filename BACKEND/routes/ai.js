...
const normalizeCseSecurities = (payload) => {
  if (!payload) return [];
  ...
  if (Array.isArray(payload)) {
    ...
    const possibleArrays = Object.values(payload).filter((value) => Array.isArray(value));
    if (possibleArrays.length > 0) {
      items = possibleArrays.flat();
    }
  }
  ...
  return items
    .map((item) => {
      const symbol = String(
        item.symbol || item.securitySymbol || item.ticker || item.code || item.securityId || ''
      ).trim();
      const name = String(
        item.name || item.securityName || item.companyName || item.longName || ''
      ).trim();
      ...
      return {
        ...
        type: 'equity',
        currency: 'LKR',
        note: 'CSE security list (delayed)'
      };
    })
    .filter(Boolean);
};

const fetchCseSecurities = async () => {
  const endpoints = ['listedCompanies', 'cntSecurity'];
  ...
  for (const endpoint of endpoints) {
    ...
    const securities = normalizeCseSecurities(payload);
    if (securities.length) {
      return { ok: true, data: securities, endpoint };
    }
  }

  return { ok: false, reason: 'cse-empty' };
};
...
router.get('/market/watchlist', authenticateToken, async (req, res) => {
  try {
    ...
    const cse = await fetchCseSecurities().catch((error) => {
      console.warn('CSE API unavailable:', error.message);
      return { ok: false, reason: 'cse-error' };
    });
    ...
    if (!apiKey) {
      if (cse.ok) {
        return res.json({
          ...
          data: {
            sriLanka: cse.data,
            abroad: fallbackMarketData.abroad
          },
          note: `CSE endpoint: ${cse.endpoint || 'unknown'}`
        });
      }
    ...
    const cseList = cse.ok ? cse.data : [];
    const mergedSriLanka = [
      ...sriLanka,
      ...cseList.filter((item) => !sriLanka.some((existing) => existing.symbol === item.symbol))
    ];

    return res.json({
      success: true,
      source: 'finnhub-watchlist',
      data: { sriLanka: mergedSriLanka, abroad },
      note: cse.ok ? `CSE endpoint: ${cse.endpoint || 'unknown'}` : undefined
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
